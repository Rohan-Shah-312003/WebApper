...
function buildTrayIcon() {
	...
	// directly encode the png to base64 string
...
function launchWebApp(webApp) {
	if (launchedWindows.has(webApp.id)) {
		const app_tab = launchedWindows.get(webApp.id);
		if (!app_tab.isDestroyed()) {
			app_tab.show();
			app_tab.focus();
			return;
		}
	...
	function push(extra = {}) {
		...
		const state = {
			canBack:
				...
				siteView.webContents.canGoBack(),
			canForward:
				!siteView.webContents.isDestroyed() &&
				siteView.webContents.canGoForward(),
			loading:
				!siteView.webContents.isDestroyed() &&
				siteView.webContents.isLoading(),
