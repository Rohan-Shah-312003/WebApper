...
// ── Gallery data ───────────────────────────────────────────────────────────────
const GALLERY = [
  { name: 'Gmail', url: 'https://mail.google.com', icon: '📧', mode: 'standard' },
  { name: 'Google Calendar', url: 'https://calendar.google.com', icon: '📅', mode: 'standard' },
  { name: 'Google Drive', url: 'https://drive.google.com', icon: '📁', mode: 'tabbed' },
  { name: 'Google Docs', url: 'https://docs.google.com', icon: '📝', mode: 'tabbed' },
  { name: 'Notion', url: 'https://www.notion.so', icon: '🗒', mode: 'standard' },
  ...
  { name: 'ChatGPT', url: 'https://chat.openai.com', icon: '🤖', mode: 'standard' },
  { name: 'Claude', url: 'https://claude.ai', icon: '🧠', mode: 'standard' },
  { name: 'YouTube', url: 'https://youtube.com', icon: '▶️', mode: 'standard' },
  { name: 'Spotify Web', url: 'https://open.spotify.com', icon: '🎵', mode: 'standard' },
  ...
  { name: 'WhatsApp Web', url: 'https://web.whatsapp.com', icon: '💚', mode: 'standard' },
...
function modeClass(mode) {
  return { standard: 'badge-standard', tabbed: 'badge-tabbed', incognito: 'badge-incognito' }[mode] || 'badge-standard';
}
...
function renderLibrary() {
  ...
  const filtered = apps.filter(a => {
    const matchSearch = !search || a.name.toLowerCase().includes(search) || a.url.toLowerCase().includes(search);
...
function openModal(prefill = {}) {
  ...
  document.getElementById('modalTitle').textContent = editingId ? 'Edit App' : 'New App';
  document.getElementById('btnSave').textContent = editingId ? 'Save Changes' : 'Create App';

  document.getElementById('fieldUrl').value = prefill.url || '';
  document.getElementById('fieldName').value = prefill.name || '';
  document.getElementById('fieldWhitelist').value = (prefill.whitelist || []).join('\n');
  ...
  document.querySelectorAll('.mode-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.mode === (prefill.mode || 'standard'));
  });
...
async function saveApp() {
  ...
  if (!url) { document.getElementById('fieldUrl').focus(); return; }
  ...
  const appData = {
    ...
    name,
    url: finalUrl,
    icon: modalIconDataUrl ? null : (document.getElementById('iconEmoji').textContent || '🌐'),
...
function suggestNameFromUrl(url) {
  ...
  urlDebounce = setTimeout(() => {
    ...
    try {
      ...
      if (parts[0]) nameField.value = parts[0].charAt(0).toUpperCase() + parts[0].slice(1);
    } catch {}
  }, 400);
}
...
async function init() {
  ...
  // New app buttons
  document.getElementById('btnNewApp').addEventListener('click', () => openModal());
  document.getElementById('btnNewAppEmpty').addEventListener('click', () => openModal());
  ...
  document.getElementById('btnFetchFavicon').addEventListener('click', async () => {
    const url = document.getElementById('fieldUrl').value.trim();
    if (!url) return;
    ...
    if (suggestions && suggestions[0]) {
      ...
      img.onload = () => {
        ...
        try { modalIconDataUrl = canvas.toDataURL('image/png'); } catch {}
      };
