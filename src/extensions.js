		...
		https
			.get(url, { headers: { "User-Agent": "Mozilla/5.0" } }, res => {
				if (
					...
					return resolve(
						httpsGet(res.headers.location, redirectCount + 1),
					);
				}
...
async function installExtension(storeUrlOrId) {
	...
	try {
		if (fs.existsSync(extDestDir))
			fs.rmSync(extDestDir, { recursive: true });
	...
	loadExtensionIntoSessions(extDestDir);
	...
	for (const partition of loadedPartitions) {
		try {
			...
			if (!alreadyLoaded.has(extId)) {
				loadExtensionIntoSession(ses, extDir);
			}
		} catch {}
	}
}
...
async function loadAllExtensionsIntoPartition(partition) {
	...
	for (const ext of exts) {
		if (fs.existsSync(ext.dir) && !alreadyLoaded.has(ext.id)) {
			loadExtensionIntoSession(ses, ext.dir);
		}
	}
}
